...
Natural Show provides three services for programmatic control, allowing you to integrate with automations and scripts.

## natural_show.apply

Applies the current Natural Show settings to lights on demand. Useful for forcing an immediate update or applying settings to lights that aren't in the regular adaptation cycle.
...
service: natural_show.apply
data:
  entity_id: switch.natural_show_bedroom
  transition: 5
...
Marks or unmarks a light as "manually controlled". When a light is marked as manually controlled, Natural Show will not adjust it until the manual control flag is cleared.
...
Natural Show also fires events that you can use in automations.

### natural_show.manual_control
...
| `switch` | The Natural Show switch entity |
