...
# Path to the Lovelace card asset served at /adaptive_lighting/www/
_CARD_URL  = "/adaptive_lighting/www"
_CARD_FILE = Path(__file__).parent / "www" / "adaptive-lighting-config-card.js"
...
class AdaptiveLightingConfigView(HomeAssistantView):
    """REST endpoint used by the Lovelace card to update a config entry's lights list.

    PUT /api/adaptive_lighting/config/<entry_id>
    ...
    url  = "/api/adaptive_lighting/config/{entry_id}"
    name = "api:adaptive_lighting:config"
    requires_auth = True
...
async def async_setup(hass: HomeAssistant, config: dict[str, Any]) -> bool:
    ...
    if not _CARD_REGISTERED:
        ...
        # Serve the Lovelace card JS from /adaptive_lighting/www/
        ...
        hass.http.register_view(AdaptiveLightingConfigView)
        _LOGGER.debug(
            "Adaptive Lighting: card served at %s/adaptive-lighting-config-card.js",
            _CARD_URL,
        )
