...
const AL_DOMAIN    = 'natural_show';
const CARD_TAG     = 'natural-show-config-card';

class NaturalShowConfigCard extends HTMLElement {
      ...
      <ha-card>
        <div class="header">
          <div class="header-left">
            ...
            <span class="header-title">Natural Show</span>
          </div>
  ...
  async _saveLights() {
    if (!this._selectedSwitch) {
      this._toast('Select a Natural Show instance first', 'warn');
      return;
    }
    ...
    try {
      ...
      if (entryId) {
        ...
        await this._hass.callApi('PUT', `natural_show/config/${entryId}`, {
  ...
  async _resolveEntryId(entityId) {
    try {
      ...
      const slug = entityId.replace(/^switch\.natural_show_/, '').replace(/_/g, ' ');
...
customElements.define(CARD_TAG, NaturalShowConfigCard);

window.customCards = window.customCards || [];
if (!window.customCards.find(c => c.type === CARD_TAG)) {
  window.customCards.push({
    type: CARD_TAG,
    name: 'Natural Show Config',
    description: 'Area/room light picker + circadian preview for Natural Show',
    preview: false,
    documentationURL: 'https://github.com/Ctrlable/natural-show',
  });
}
