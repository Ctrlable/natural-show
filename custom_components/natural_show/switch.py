...
def _switches_from_service_call(
    ...
    if not lights and not switch_entity_ids:
        msg = (
            "natural-show: Neither a switch nor a light was provided in the service call."
            " If you intend to adapt all lights on all switches, please inform the"
            " developers at https://github.com/Ctrlable/natural-show about your"
            " use case. Currently, you must pass either an natural-show switch or"
            " the lights to an `natural_show` service call."
        )
    ...
    if switch_entity_ids is not None:
        if len(switch_entity_ids) > 1 and lights:
            msg = (
                "natural-show: Cannot pass multiple switches with lights argument."
    ...
    msg = (
        "natural-show: Incorrect data provided in service call."
...
async def handle_change_switch_settings(
    ...
    _LOGGER.debug(
        "Called 'natural_show.change_switch_settings' service with '%s'",
        data,
    )
...
async def async_setup_entry(  # noqa: PLR0915
    ...
    """Set up the NaturalShow switch."""
    assert hass is not None
    data = hass.data[DOMAIN]
    assert config_entry.entry_id in data
    _LOGGER.debug(
        "Setting up NaturalShow with data: %s and config_entry %s",
        data,
        config_entry,
    )
    if (  # Skip deleted YAML config entries or first time YAML config entries
        ...
        _LOGGER.warning(
            "Deleting NaturalShow switch '%s' because YAML"
    ...
    if (manager := data.get(ATTR_ADAPTIVE_LIGHTING_MANAGER)) is None:
        manager = NaturalShowManager(hass)
    ...
    async def handle_apply(service_call: ServiceCall) -> None:
        ...
        data = service_call.data
        _LOGGER.debug(
            "Called 'natural_show.apply' service with '%s'",
            data,
        )
    ...
    async def handle_set_manual_control(service_call: ServiceCall) -> None:
        ...
        data = service_call.data
        _LOGGER.debug(
            "Called 'natural_show.set_manual_control' service with '%s'",
            data,
        )
        switches = _switches_from_service_call(hass, service_call)
        lights = data[CONF_LIGHTS]
...
def _supported_features(hass: HomeAssistant, light: str) -> set[str]:
    ...
    # comment https://github.com/Ctrlable/natural-show/issues/112#issuecomment-836944011
...
class NaturalShowSwitch(SwitchEntity, RestoreEntity):
    ...
    def __init__(
        ...
        hass: HomeAssistant,
        config_entry: ConfigEntry,
        manager: NaturalShowManager,
        sleep_mode_switch: SimpleSwitch,
        adapt_color_switch: SimpleSwitch,
        adapt_brightness_switch: SimpleSwitch,
    ) -> None:
        """Initialize the Natural Show switch."""
    ...
    def name(self) -> str:
        ...
        return f"Natural Show: {self._name}"
    ...
    def _call_on_remove_callbacks(self) -> None:
        ...
        # See https://github.com/Ctrlable/natural-show/issues/658
        self._remove_listeners()
        try:
    ...
    def create_context(
        ...
        """Create a context that identifies this Natural Show instance."""
    ...
    async def _respond_to_off_to_on_event(
        ...
        if (
            ...
            and from_turn_on
            # natural_show.apply can turn on light, so check this is not our context
    ...
    def fire_manual_control_event(
        ...
        _LOGGER.debug(
            "'natural_show.manual_control' event fired for %s for light %s",
            self.entity_id,
            light,
        )
...
class SimpleSwitch(SwitchEntity, RestoreEntity):
    ...
    def __init__(
        ...
        self._name = f"Natural Show {which}: {self._config_name}"
    ...
    def device_info(self) -> DeviceInfo:
        ...
        return DeviceInfo(
            ...
            name=f"Natural Show: {self._config_name}",
            entry_type=DeviceEntryType.SERVICE,
        )
...
class NaturalShowManager:
    ...
    def __init__(self, hass: HomeAssistant) -> None:
        """Initialize the NaturalShowManager that is shared among all switches."""
        assert hass is not None
        self.hass = hass
    ...
    async def turn_on_off_event_listener(self, event: Event) -> None:
        ...
        async def on(eid: str, event: Event) -> None:
            ...
            # Fix for https://github.com/Ctrlable/natural-show/issues/1378
        ...
        if new_on:
            ...
            if is_our_context(new_on.context):
                if (
                    ...
                    _LOGGER.debug(
                        "NaturalShowManager: State change event of '%s' is already"
                    ...
                    _LOGGER.debug(
                        "NaturalShowManager: New adapt '%s' found for %s",
                        new_on,
                        entity_id,
                    )
    ...
    async def update_manually_controlled_from_event(
        ...
        _LOGGER.debug(
            ...
            " natural_show integration (context.id='%s'), the Adaptive"
    ...
    def _off_to_on_state_event_is_from_turn_on(
        ...
        # Natural Show should never turn on lights itself
        if is_our_context(off_to_on_event.context) and not is_our_context(
            off_to_on_event.context,
            "service",  # natural_show.apply is allowed to turn on lights
        ):
            _LOGGER.warning(
                "Detected an 'off' → 'on' event for '%s' with context.id='%s',"
                " triggered by the natural_show integration itself,"
                " which *should* not happen. If you see this please submit an issue with"
                " your full logs at https://github.com/Ctrlable/natural-show",
                entity_id,
...
class _AsyncSingleShotTimer:
    ...
    def start(self) -> None:
        ...
        # See: https://github.com/Ctrlable/natural-show/issues/1272
