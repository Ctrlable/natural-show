...
Navigate to the `natural-show` repo folder and execute the following command.

**Important:** Mount the entire repo (`-v $(pwd):/app`), not individual directories, or the symlinks will break.

Linux / MacOS / Windows PowerShell:
```bash
docker run -v ${PWD}:/app basnijholt/natural-show:latest
```

- In windows command prompt, the command is:
  ```bash
  docker run -v %cd%:/app basnijholt/natural-show:latest
  ```

This command will download the Docker image from [the natural-show Docker Hub repo](https://hub.docker.com/r/basnijholt/natural-show) and run the tests.

If you prefer to build the image yourself, use the following command:

```bash
docker build -t basnijholt/natural-show:latest --no-cache --progress=plain .
```
...
```bash
docker run -v $(pwd):/app basnijholt/natural-show:latest --show-capture=log --log-format="%(asctime)s %(levelname)-8s %(name)s:%(filename)s:%(lineno)s %(message)s" --log-date-format="%H:%M:%S" tests/components/natural_show/
```
